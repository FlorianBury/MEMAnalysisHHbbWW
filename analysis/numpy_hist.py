...
class NumpyHist:
    ...
    def __init__(self,e,w,s2,name=None):
        ...
        if self.ndim == 1:
            ...
            if self._w.shape[1] != self._e[1].shape[0]-1:
                raise ValueError(f'Histogram y-axis content has {self._w.shape[1]} bins, but there are {self._e[1].shape[0]} bin edges ')
            if self._w.shape[2] != self._e[2].shape[0]-1:
                raise ValueError(f'Histogram y-axis content has {self._w.shape[2]} bins, but there are {self._e[2].shape[0]} bin edges ')
        else:
    ...
    def widths(self):
        ...
        return self._widths(self._e)

    def _widths(self,e):
        if isinstance(e,np.ndarray): # 1D
            widths = e[1:]-e[:-1]
        elif isinstance(e,list):
            if len(e) == 1: # 1D
                widths = e[0][1:]-e[0][:-1]
            elif len(e) == 2: # 2D
                widths = np.dot((e[0][1:]-e[0][:-1]).reshape(-1,1),
                                (e[1][1:]-e[1][:-1]).reshape(1,-1))
            elif len(e) == 3: # 3D
                widths = np.dot(np.expand_dims(np.dot((e[0][1:]-e[0][:-1]).reshape(-1,1),
                                                      (e[1][1:]-e[1][:-1]).reshape(1,-1)),
                                               axis=2),
                                (e[2][1:]-e[2][:-1]).reshape(1,-1))
            else:
                raise NotImplementedError
        else:
            raise RuntimeError(f'Format {type(e)} not understood')
        if (widths <= 0.).any():
    ...
    def getFromRoot(cls,h):
        ...
        elif h.__class__.__name__.startswith('TH3'):
            e,w,s = CppInterface.getContent3D(h)
        else:
    ...
    def fillHistogram(self,name=None):
        ...
        if self.ndim == 1:
            ...
            return CppInterface.fillHistogram3D(self.e,self.w,self.s,name)
        else:
            raise NotImplementedError(f'Dimension {self.ndim} has not been coded')
    ...
    def compareAxes(self,other):
        ...
        if self.ndim == 1:
            ...
            if e1[1].shape[0] != e2[1].shape[0]:
                raise RuntimeError(f'Y axes have different lenghts in the two histograms : {e1[1].shape[0]-1} versus {e2[1].shape[0]-1}')
            if not np.isclose(e1[1],e2[1]).all():
                raise RuntimeError(f'Different y axes in two histograms : {e1[1]} versus {e2[1]}')
            if e1[2].shape[0] != e2[2].shape[0]:
                raise RuntimeError(f'Z axes have different lenghts in the two histograms : {e1[2].shape[0]-1} versus {e2[2].shape[0]-1}')
            if not np.isclose(e1[2],e2[2]).all():
                raise RuntimeError(f'Different z axes in two histograms : {e1[2]} versus {e2[2]}')
        else:
            raise NotImplementedError(f'Dimension {self.ndim} has not been coded')
    ...
    def __add__(self,other):
        """
            Implements the addition with another NumpyHist or number
        ...
        if isinstance(other,float) or isinstance(other,int):
            ...
            raise RuntimeError(f'Format {type(other)} not understood')

    def __sub__(self,other):
        """
            Implements the subtraction with another NumpyHist or number
            First, check the axes, then subtract the content linearly, and errors quadratically
            Then returns the new results
        """
        if isinstance(other,float) or isinstance(other,int):
            return NumpyHist(self._e,
    ...
    def __mul__(self,other):
        """
            Implements the multiplication with another NumpyHist or number
            First, check the axes, then multiply the content, and errors 
                x**2 * dy**2 + y**2 * dx**2
            Then returns the new results
        """
        if isinstance(other,float) or isinstance(other,int):
            ...
            return NumpyHist(self._e,
                             self._w * other._w,
                             self._w**2 * self._s2 * + other._w**2 * other._s2,
                             self._name)
        else:
            raise RuntimeError(f'Format {type(other)} not understood')
    ...
    def __truediv__(self,other):
        ...
        if isinstance(other,float) or isinstance(other,int):
            ...
            # Return multiplication #
            if np.any(other == 0.):
                return RuntimeError('Cannot divide by zero in bin content')
            return NumpyHist(self._e,
                             self._w / other._w,
                             (self.s / other._w)**2 + (self._w/other._w)**2 * (other.s/other._w)**2,
                             self._name)
        else:
            raise RuntimeError(f'Format {type(other)} not understood')
    ...
    def add(self,other):
        """
            Implements the addition by modifying the self
        """
        newObj = self + other
    ...
    def sub(self,other):
        """
            Implements the subtraction by modifying the self
        """
        newObj = self - other
        self.__dict__.update(newObj.__dict__)
    ...
    def mul(self,other):
        """
            Implements the multiplication by modifying the self
        """
        newObj = self * other
        self.__dict__.update(newObj.__dict__)
    ...
    def div(self,other):
        """
            Implements the division by modifying the self
        """
        newObj = self / other
        self.__dict__.update(newObj.__dict__)
    ...
    def rebin(self,ne,average_bins=False):
        """
        ...
        if self.ndim == 1:
            ...
            for i in range(nw.shape[0]):
                if average_bins:
                    rebin_id, = np.where(idx == i)
                    rebin_e = np.append(self._e[rebin_id],self._e[rebin_id[-1]+1])
                    widths = self._widths(rebin_e)
                    nw[i] += (self._w[rebin_id] * widths).sum() / widths.sum()
                    ns2[i] += (self._s2[rebin_id] * widths).sum() / widths.sum()
                else:
        ...
        # 2D method #
            ...
            for ix in range(0,nw.shape[0]):
                for iy in range(0,nw.shape[1]):
                    if average_bins:
                        rebin_idx, = np.where(idx==ix)
                        rebin_idy, = np.where(idy==iy)
                        rebin_ex = np.append(self._e[0][rebin_idx],self._e[0][rebin_idx[-1]+1])
                        rebin_ey = np.append(self._e[1][rebin_idy],self._e[1][rebin_idy[-1]+1])
                        widths = self._widths([rebin_ex,rebin_ey])
                        nw[ix,iy] += (self._w[np.ix_(idx==ix,idy==iy)] * widths).sum() / widths.sum()
                        ns2[ix,iy] += (self._s2[np.ix_(idx==ix,idy==iy)] * widths).sum() / widths.sum()
                    else:
        ...
        # 3D method #
            ...
            self.compareRebinAxes(self._e[0],ne[0])
            self.compareRebinAxes(self._e[1],ne[1])
            self.compareRebinAxes(self._e[2],ne[2])
            # Get indices #
            x = (self._e[0][1:]+self._e[0][:-1])/2
            y = (self._e[1][1:]+self._e[1][:-1])/2
            z = (self._e[2][1:]+self._e[2][:-1])/2
            idx = np.digitize(x,ne[0]) - 1
            idy = np.digitize(y,ne[1]) - 1
            idz = np.digitize(z,ne[2]) - 1
            # Perform aggregation #
            nw = np.zeros((ne[0].shape[0]-1,ne[1].shape[0]-1,ne[2].shape[0]-1))
            ns2 = np.zeros((ne[0].shape[0]-1,ne[1].shape[0]-1,ne[2].shape[0]-1))
            for ix in range(0,nw.shape[0]):
                for iy in range(0,nw.shape[1]):
                    for iz in range(0,nw.shape[2]):
                        if average_bins:
                            rebin_idx, = np.where(idx==ix)
                            rebin_idy, = np.where(idy==iy)
                            rebin_idz, = np.where(idz==iz)
                            rebin_ex = np.append(self._e[0][rebin_idx],self._e[0][rebin_idx[-1]+1])
                            rebin_ey = np.append(self._e[1][rebin_idy],self._e[1][rebin_idy[-1]+1])
                            rebin_ez = np.append(self._e[2][rebin_idz],self._e[2][rebin_idz[-1]+1])
                            widths = self._widths([rebin_ex,rebin_ey,rebin_ez])
                            nw[ix,iy,iz] += (self._w[np.ix_(idx==ix,idy==iy,idz==iz)] * widths).sum() / widths.sum()
                            ns2[ix,iy,iz] += (self._s2[np.ix_(idx==ix,idy==iy,idz==iz)] * widths).sum() / widths.sum()
                        else:
                            nw[ix,iy,iz] += self._w[np.ix_(idx==ix,idy==iy,idz==iz)].sum()
                            ns2[ix,iy,iz] += self._s2[np.ix_(idx==ix,idy==iy,idz==iz)].sum()
        else:
            raise NotImplementedError
...
if __name__ == '__main__':
    #w = np.arange(2*3*4).reshape(3,4,2)
    #s2 = w/10
    #e = [np.array([0,2,5,6]),np.array([0,3,6,9,12]),np.array([0,1,3])]
    #nph = NumpyHist(e,w,s2)
    #n2 = nph.rebin([np.array([0,5,6]),np.array([0,3,6,9,12]),np.array([0,1,3])])

    w = np.arange(9).reshape(3,3)
    s2 = w/10
    e = [np.array([0,1,3,6]),np.array([0,1,3,6])]
    nph = NumpyHist(e,w,s2)
    n2 = nph.rebin([np.array([0,3,6]),np.array([0,1,6])],True)
    embed()
